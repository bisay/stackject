        ...
        <nav className="nav-glass" style={{ padding: '1rem 3rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'relative' }}>
            ...
            <div className={`nav-menu ${isMenuOpen ? 'open' : ''}`} style={{ display: 'flex', gap: '2rem', alignItems: 'center' }}>
                ...
                <div className="menu-auth" style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                    ...
                    {!user ? (
                        <>
                            <Link href="/login" style={{ textDecoration: 'none', color: 'var(--text-muted)', fontWeight: 600, fontSize: '0.95rem' }}>Login</Link>
                            <Link href="/register" className="btn-primary" style={{ textDecoration: 'none', padding: '10px 24px', borderRadius: '100px' }}>Sign Up</Link>
                        </>
                    ) : (
                        <div className="user-actions" style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                            <Link href="/dashboard" className="glass-card mobile-dashboard-btn" style={{ padding: '8px 16px', borderRadius: '100px', textDecoration: 'none', color: 'var(--text-main)', fontWeight: 600, fontSize: '0.9rem', boxShadow: 'none', border: '1px solid var(--glass-border)', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                <LayoutDashboard size={16} /> Dashboard
                            </Link>
                            <button onClick={logout} className="mobile-logout-btn" style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', display: 'flex' }} title="Log Out">
                                <LogOut size={20} />
                            </button>
                        </div>
                    )}
