...
import { MessageSquare, Calendar, User, ArrowLeft, Send, Trash2, Image as ImageIcon, Heart, Share, MoreHorizontal } from 'lucide-react';
...
const CommentThread = ({ comment, user, handleReplyClick, handleLike, expandedThreads, toggleThread, discussionId, depth = 0, replyingToId, submitReply, cancelReply }: any) => {
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', overflow: 'hidden' }}>
                            <span style={{ fontWeight: 700, fontSize: '0.95rem', whiteSpace: 'nowrap', textOverflow: 'ellipsis', overflow: 'hidden' }} className="hover-underline">{comment.author.name}</span>
                            ...
                            {hasChildren && (
                                ...
                                >
                                    <div style={{ padding: '2px 8px', borderRadius: '12px', background: 'rgba(255,255,255,0.05)', color: expandedThreads[comment.id] ? 'var(--primary)' : 'inherit', fontSize: '0.75rem' }}>
                                        {expandedThreads[comment.id] ? 'Hide' : `${comment.children.length} replies`}
                                    </div>
                                </div>
                            )}
                        </div>
                ...
                <div style={{ padding: '24px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column' }}>
                            <span style={{ fontWeight: 700, fontSize: '1.1rem', color: 'var(--text-main)' }}>{discussion.author.name}</span>
